    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'add':
                    if (msg.name?.trim()) {
                        await this.profileManager.addProfile(msg.name.trim());
                        this.sendProfiles();
                    }
                    break;
                case 'remove':
                    if (msg.name?.trim()) {
                        await this.profileManager.removeProfile(msg.name.trim());
                        this.sendProfiles();
                    }
                    break;
                case 'clear':
                    await this.profileManager.clearProfiles();
                    this.sendProfiles();
                    break;
            }
        });
...
exports.MavenProfilesView = MavenProfilesView;
MavenProfilesView.viewId = 'mavenProfiles';
function getNonce() {
    let t = '';
...
//# sourceMappingURL=MavenProfilesView.js.map