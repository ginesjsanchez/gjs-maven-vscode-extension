 ...
 */
export class HeaderAssociationManager {
    ...
    activate(): void {
        ...
        this.context.subscriptions.push(
            MavenProjectContext.onDidChangeActiveModule(module => void this.apply(module))
        );

        void this.reconcile();
        void this.apply(MavenProjectContext.activeModule);
    }

    private enabled(): boolean {
        return vscode.workspace.getConfiguration('gjsMaven').get<boolean>('cpp.associateHeaders', true);
    }
    ...
    private async apply(module: MavenModuleInfo | undefined): Promise<void> {
        ...
        for (const each of stale) { delete next[each]; }

        if (this.same(scoped, next)) { return; }

        await this.write(next, folder.uri);

        const record = { ...owned };
        for (const each of stale) { delete record[each]; }
        if (isC) { record[key] = module.projectDir; } else { delete record[key]; }
    ...
    private async reconcile(): Promise<void> {
        for (const folder of vscode.workspace.workspaceFolders ?? []) {
            ...
            const next = { ...scoped };
            const record = { ...owned };
            for (const each of stale) { delete next[each]; delete record[each]; }
    ...
    private staleKeys(owned: Record<string, string>, associations: Record<string, string>): string[] {
        return Object.keys(owned).filter(key =>
            key in associations && !fs.existsSync(path.join(owned[key], 'pom.xml')));
    }
     ...
     */
    private keyFor(folderDir: string, projectDir: string): string {
        const relative = path.relative(folderDir, projectDir).replace(/\\/g, '/');
        return relative ? `**/${relative}/**/*.h` : '**/*.h';
    }

    private owned(): Record<string, string> {
        return { ...this.context.workspaceState.get<Record<string, string>>(OWNED_KEYS, {}) };
    }
     ...
     */
    private scopedAssociations(folderUri: vscode.Uri): Record<string, string> {
        const info = vscode.workspace.getConfiguration('files', folderUri)
            .inspect<Record<string, string>>('associations');
    ...
    private async write(associations: Record<string, string>, folderUri: vscode.Uri): Promise<void> {
    ...
    private same(a: Record<string, string>, b: Record<string, string>): boolean {
