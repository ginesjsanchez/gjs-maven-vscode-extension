...
function select(all: Archetype[], filter: string): { rows: Archetype[]; matches: number; total: number } {
...
export class MavenArchetypesView implements vscode.WebviewViewProvider {
    ...
    private local: Archetype[] = [];
    private global: Archetype[] = [];
    ...
    private load(): boolean {
        ...
        if (!this.existsDir(localRepo)) { return false; }
    ...
    private sendArchetypes(localFilter = '', globalFilter = ''): void {
        ...
        if (!this.load()) {
            ...
            this.webviewView.webview.postMessage({
                command: 'pending',
                reason: MavenProjectContext.globalConfig.localRepositoryError
            });
            return;
        }
